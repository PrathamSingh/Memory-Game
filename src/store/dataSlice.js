...
const dataSice = createSlice({
  ...
  reducers: {
    setCellOnClick: (state, { payload }) => {
      ...
      const memoryGridValueAtClickedPosition =
        state.memoryGrid[payload.rowIndex][payload.colIndex];
      const userGridValueAtClickedPosition =
        state.userGrid[payload.rowIndex][payload.colIndex];

      if (
        memoryGridValueAtClickedPosition === 1 &&
        userGridValueAtClickedPosition === 0
      ) {
